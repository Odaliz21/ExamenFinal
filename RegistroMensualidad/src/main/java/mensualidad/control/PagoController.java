...
public class PagoController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        recargaDatosService = new ScheduledService<>() {
            protected Task<Void> createTask() {
                return new Task<>() {
                    @Override
                    protected Void call() throws Exception {
                        // Reload data from the database and update the table
                        cargarPagos();
                        return null;
                    }
                };
        ...
        recargaDatosService.setPeriod(Duration.seconds(5)); // Configure the time interval in seconds
